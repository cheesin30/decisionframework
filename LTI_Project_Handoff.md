...
| `Workshop_Tool_Generator.html` | **Replica factory** for other Capital Learning workshops: pick the current LTI tool as source, enter name/code/SharePoint folder/Form pre-fill link/default touchpoints → downloads e.g. `TKO_Engagement_Calendar_QR_v1.html`. Works by anchored replacements on the live source (8 integrity checks, refuses to emit if an anchor is missing), tokenises user values so renames can't corrupt them, and namespaces `BUILD_ID` so replicas never share localStorage. If the calendar tool changes shape (`BUILD_ID`, `DEFAULT_REMINDERS`, `FORM_DEFAULT_TEMPLATE`, `formState` defaults, `_LTI_Follow_Up_`, the folder path literal), update the generator's `ANCHORS`/`buildReplica`. |
