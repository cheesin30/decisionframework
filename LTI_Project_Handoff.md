...
6. **No Power Automate premium.** The `Execute JavaScript Code` action (which would parse the `.ics` in the flow) is premium and unavailable → we push parsing into the tool and export ready-made JSON instead.
